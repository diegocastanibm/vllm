...
all_batch_sizes = [
    item[0] for group in [data1, data2, data3, data4] for item in group
]
bin_edges = np.arange(min(all_batch_sizes),
                      max(all_batch_sizes) + 2, 1)  # 20 bins from min to max

hist_data = [[item[1] for item in group if item[0] in bin_edges]
             for group in [data1, data2, data3, data4]]
